...
def get_sot_cached(fixture_id: int):
    ...
    if cached and (now_ts - cached["ts"] < STATS_MIN_INTERVAL_SEC):
        return cached["home"], cached["away"]

    h, a = get_stats_sot(fixture_id)
...
def resolve_goal1h_at_ht_if_possible():
    ...
    for a in alerts.values():
        ...
        a["resolved"] = True
        a["result"] = "WIN" if (ht_home + ht_away) >= 1 else "LOSE"
        changed = True
...
def check_alerts_for_match(match: dict, allowed_league_ids: set):
    ...
    if last_stats_ts > 0 and (now_ts - last_stats_ts) < STATS_MIN_INTERVAL_SEC:
        ...
        h_sot, a_sot = get_sot_cached(fixture_id)
        TRACKED_MATCHES[fixture_id]["last_stats_ts"] = now_ts

    sot_total = h_sot + a_sot
    ...
    if 20 <= minute <= 30 and score_str == "0 - 0" and sot_total >= 3:
        ...
        if not is_already_sent(fixture_id, code):
            msg = build_premium_message(
                ALERT_META[code]["title"], league_name, country,
                home_name, away_name, minute, status_short,
                score_str, h_sot, a_sot, ALERT_META[code]["pick"]
            )
            if send_telegram(msg):
